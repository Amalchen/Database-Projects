...
I didn't know how to use "JOIN" back then, and used another way to implement it, building the string in a smart way. The string built was something likeI had "[...] WHERE `menu`.`id` == 1 && `menu`.`id` == 1 [...].
This didn't work.
...
Notes:
1) After ordering, when clicking "Checkout", the Order DB-table will clear your orders and your reservation (if it exists).
2) Smoker, non-smoker option was never implemented.
3) I have two types of tables that fit 4 or 6 people.
4) The difficult part was the coupling of tables.
I did the silliest/simplest sollution.
